...
func _on_area_entered(p_area: Area2D) -> void:
	if p_area.is_in_group("interactive") and p_area.has_method("interact_enter"):
		p_area.interact_enter(get_parent())

func _on_area_exited(p_area: Area2D) -> void:
	if p_area.is_in_group("interactive") and p_area.has_method("interact_leave"):
...
func _process_interact() -> void:
	for node in get_overlapping_areas():
		if node.is_in_group("interactive") and node.has_method("interact"):
